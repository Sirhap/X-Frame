...
3. Place with `xsxb_place_image`: `target_anchor` `{view, cells, derive}` or `{view, cells, snap}` (or `x_from` / `y_from`), `object_anchor` `alpha_*` / cell derive / `snap` / **`measure_t`**, and `scale` `relative` or `physical` from the selected span. Prefer `snap: "alpha_centroid"` when the subject (e.g. a hand) sits off-center inside the cell — MCP resolves the opaque **mass** mean (not just the bbox mid) into `receipt.resolved` `{x,y}`. For swords/props, prefer `object_anchor: { measure_t: 0.15 }` (pommel→tip; ~0.12–0.2 for grips) instead of object cell mid-points — cell centers miss the brown handle and leave the hilt visibly off the fist. Optional `nudge: {dx, dy}` tweaks that grounded point in pixels. Never invent freehand pixel x,y. Do not scale from full image width per meter. Aspect mismatch warns and does not stretch.
4. **Held fast path:** hand cell + `snap: "alpha_centroid"` + `measure_t` on the weapon + `under_target` + forearm-aligned `rotation` + body-span `scale` + look at `verify_overlay_path`. Skip inventing object-grid midpoints for the grip.
5. `verify_overlay` defaults true: look at `verify_overlay_path` after placing. Set `verify_overlay: false` only when chaining bulk places.
6. `layer`: `front` (default) paints the object on top; `under_target` restores opaque pixels inside the `target_anchor` cell union so a grip can sit in a palm while pixels outside that box stay in front; `behind` restores every opaque target pixel. `rotation` is clockwise degrees around the object anchor (screen y-down). Held objects follow pose physics, not source-upright: rotation 0 is the generated PNG (often the heavy head down). Rotate around the grip so the mass/striking end faces the figure's facing or attack side and the shaft follows the forearm, not world-vertical. Scale from the body span. The tool does not redraw a hand; if the head clips the canvas, pad that edge or grip closer to the head.
7. Example only: standing a person in a doorway is cell `bottom_center` plus physical width — there are no door/person fields on the tools.
8. `xsxb_measure_image` `anchor=alpha_bottom` returns the same opaque-foot geometry; weapon `t` matches `object_anchor.measure_t`.
9. `xsxb_cutout` `file_path` runs the same smart-cutout on one workspace PNG (sibling `_cut.png` by default). Animation-frame cutout is unchanged.
