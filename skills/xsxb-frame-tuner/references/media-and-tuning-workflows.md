...
3. Place with `xsxb_place_image`: `target_anchor` `{view, cells, derive}` or `{view, cells, snap}` (or `x_from` / `y_from`), `object_anchor` `alpha_support` / `alpha_center` / cell derive, and `scale` `relative` or `physical` from the selected span. Prefer `snap: "alpha_center"` when the subject (e.g. a hand) sits off-center inside the cell — MCP resolves opaque pixels inside that cell union and returns `receipt.resolved` `{x,y}`. Optional `nudge: {dx, dy}` tweaks that grounded point in pixels. Never invent freehand pixel x,y. Do not scale from full image width per meter. Aspect mismatch warns and does not stretch.
