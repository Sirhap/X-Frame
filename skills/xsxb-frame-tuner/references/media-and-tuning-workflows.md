...
3. **Generic place:** overlay both images; name the contact cells on each; `target_anchor` / `object_anchor` with `snap: "alpha_centroid"` (opaque mass mean inside those cells — not cell mid-points). Refine with `crop_from` when the contact patch is small. `scale` `relative`|`physical` from a named target span. Optional `nudge: {dx,dy}` after verify. Never freehand pixel x,y.
4. `verify_overlay` defaults true: look at `verify_overlay_path` after placing. Set `verify_overlay: false` only when chaining bulk places.
5. `layer`: `front` (default), `under_target` (restore opaque target pixels inside the target cell union), or `behind`. `rotation` is clockwise degrees around the object anchor (screen y-down); set it from the pose you see. The tool does not redraw either sprite.
6. Optional long-axis helper: `object_anchor.measure_t` / `xsxb_measure_image` when you want a fraction along an elongated opaque shape — same for any such PNG, not a special product path.
7. Example only: standing a figure on a marked region is cell ids plus a physical width — there are no domain-specific place fields.
8. `xsxb_measure_image` `anchor=alpha_bottom` returns opaque-foot geometry.
