...
2. `xsxb_cutout` (omit sliders for the shared smart profile) so loop search is not poisoned by a keyed background.
...
4. Look at the preview and `xsxb_reorganize_frames` with `loop.recommended.order` or `motion.order`.
