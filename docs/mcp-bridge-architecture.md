...
当前仓库已经提供本地 STDIO MCP Server，并与工作台共用项目存储、附件、拖影、导入、调参、导出和 Godot 同步模块。Agent 可通过 34 个 `xsxb_*` 工具完成生产闭环；视觉工具同时返回有界 PNG 与 `xsxb://` 完整资源，v2 附件计划绑定审核证据并支持跨进程原子替换，所有项目写入进入可恢复 mutation journal。HTTP/WebSocket 设备配对 Bridge 仍属于后续云端连接阶段。
