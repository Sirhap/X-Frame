...
![X-Frame 深色工作台总览](product-guide-assets/02-workspace-overview.png)

## 一、可直接上架的商品文案

### 商品标题（推荐）

X-Frame Pro 激活码｜Godot 2D 帧动画调参·批量抠图·碰撞框·音效/VFX

### 短标题

X-Frame Pro 激活码｜帧动画调参工作台
...
X-Frame 是面向 Godot 2D 帧动画制作的本地优先工作台。它把图片序列导入、视频取帧、帧排序与删减、批量抠图、逐帧调参、碰撞框、音效和图片挂件等流程集中到一个可视化界面中。

首页提供“动画项目”和“快速工具”两个等权入口。需要长期编辑与调参时可进入项目；只想抠图、整理序列或切片时，可直接使用快速工具，完成后再选择下载或加入动画项目。
...
1. **一站式帧动画流程**  
   支持图片序列与视频帧导入，从素材整理、批量抠图、零散切片、预览到动画包导出，减少多工具来回切换。
...
- 交付内容：X-Frame Pro 激活码 × 1
- 发货方式：购买后按店铺页面说明自动或人工发放
- 使用入口：按发货说明提供的工作台地址或安装说明进入

## 二、购买前请确认

1. 本商品为 **X-Frame Pro 激活码**，不包含角色素材、音效素材、Godot 项目源码、软件源码或定制开发服务。
...
按发货说明打开 X-Frame。首页会显示两个同等级入口：

- **动画项目**：新建或继续可持续保存的项目，适合管理多个动画组、调参、碰撞框、音频和挂件。
- **快速工具**：无需先建项目，可直接进行批量抠图、序列处理或零散切片。

进入项目后，可在左侧查看当前项目与动画组，在中间画布预览动画，在底部时间轴选择具体帧。
...
- 底部：帧时间轴，可选择帧并调整逐帧时长
- 全局导航：“项目”和“工具”两个一级入口
- 项目内操作：批量抠图和序列整理放在“处理当前动画”区域，处理后会返回当前动画组
...
快速工具处理完成后，可以直接下载结果，也可选择“加入动画项目”。

- 默认新建动画组；如果名称重复，需要先改名，系统不会自动覆盖。
- 替换已有动画组时，必须明确选择目标。提交前会显示帧数变化，以及调参、碰撞框、音频和挂件等迁移影响，确认后才会执行。
- 一次加入多组结果时，整批内容会全部成功或全部回滚，不会留下半完成状态。
- 加入成功后当前快速会话依然保留，可继续处理或点击“打开动画组”。

**零散切片加入方式**

- **每组一个动画**：每个切片组可分别选择新建或替换目标。
- **全部合并一个动画**：按组从上到下、组内从左到右排列帧，以最大透明画布底部居中对齐。

> 快速工具中未下载、也未加入项目的大图结果只在当前会话中保留。离开页面或刷新前会出现确认；这些临时结果不会持久化。
...
您好，本商品为 X-Frame Pro 虚拟激活码。请按发货说明进入工作台，点击右上角“授权管理”，粘贴激活码并点击“立即激活”。授权设备数和有效期以您购买的规格为准。
