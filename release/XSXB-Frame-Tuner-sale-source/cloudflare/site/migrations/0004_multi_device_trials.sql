...
INSERT INTO licenses_v4
  (id, code_hash, code_ciphertext, source, plan, duration_days, max_devices,
   redeem_by, first_activated_at, expires_at, revoked_at)
SELECT id, code_hash, code_ciphertext, 'code', plan, duration_days, max_devices,
       redeem_by, first_activated_at, expires_at, revoked_at
  FROM licenses;

CREATE TABLE license_devices_v4 (
  id TEXT PRIMARY KEY,
  ...
  FOREIGN KEY (license_id) REFERENCES licenses_v4(id) ON DELETE CASCADE
);

INSERT INTO license_devices_v4
...
SELECT id, license_id, public_key, public_key_hash, NULL, device_name,
       first_ip_hash, last_ip_hash, first_country, last_country, created_at, last_seen_at, revoked_at
  FROM license_devices;

DROP TABLE license_devices;
DROP TABLE licenses;
ALTER TABLE licenses_v4 RENAME TO licenses;
ALTER TABLE license_devices_v4 RENAME TO license_devices;

CREATE TABLE automatic_trial_claims (
  id TEXT PRIMARY KEY,
