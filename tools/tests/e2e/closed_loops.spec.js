...
test("standalone export never falls back to the current project animation", async ({ page }) => {
  await page.goto("/tools/export");

  await expect(page.locator("#deliveryExportMount")).toContainText("请先导入需要导出的图片序列");
  await expect(page.locator("#deliveryExportMount #mediaExportDialog")).toHaveCount(0);
});

test("project animation copies explicitly into an independent quick-tool workset", async ({
  page,
  request,
}) => {
  const configUrl = "/api/config?project=seed-project";
  ...
  await expect(page.locator("#appConfirmMessage")).toContainText("快速工具中已有 1 帧");
  ...
  const after = await (await request.get(configUrl)).json();
...
test("applying standalone cutout returns the copied workset to organizer", async ({ page }) => {
  await page.goto("/tools/organizer");
  await page.locator("#organizerFileInput").setInputFiles([
    { name: "idle_01.png", mimeType: "image/png", buffer: ONE_PIXEL_PNG },
    { name: "idle_02.png", mimeType: "image/png", buffer: ONE_PIXEL_PNG },
  ...
  await expect(page.locator("#cutoutModal")).toBeVisible();
  await expect(page.locator(".cutoutQueueItem")).toHaveCount(2);
  await page.locator("#cutoutApplyGroup").click();
