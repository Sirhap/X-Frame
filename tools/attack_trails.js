...
function normalizeFramePhases(sticks) {
  ...
  for (const frameSticks of frames.values()) {
    ...
    while (index < frameSticks.length) {
      if (frameSticks[index].phaseMode === "manual") {
        index += 1;
        continue;
      }
      const start = index;
      ...
      for (let offset = 0; offset < count; offset += 1)
        frameSticks[start + offset].framePhase = lower + ((upper - lower) * (offset + 1)) / (count + 1);
    }
    let previous = -0.0001;
    frameSticks.forEach((stick, stickIndex) => {
      ...
      stick.framePhase = clamp(
        ...
        previous + 0.0001,
        maximum,
        (stickIndex + 1) / (frameSticks.length + 1),
      );
...
function normalizeAttackTrails(raw) {
  ...
  const presetTexture = normalizeTexture(
    source.presetTexture?.path ? source.presetTexture : DEFAULT_ATTACK_TRAIL_PRESET_TEXTURE,
  );
  const bindings = {};
...
/**
 * Resolves the workspace directory reserved for one animation's imported trail textures.
 * @param {object} projectStore Project-store adapter.
 * @param {object} project Project record.
 * @param {string} profileId Animation profile id.
 * @param {string} animationId Animation id.
 * @returns {string} Absolute animation-owned trail texture directory.
 */
function attackTrailTextureDirectory(projectStore, project, profileId, animationId) {
  return path.join(
    projectStore.projectWorkspaceDir(project),
    "attack_trails",
    slug(profileId, "profile"),
    slug(animationId, "animation"),
  );
}
...
function saveAttackTrailTexture(root, projectStore, project, payload) {
  ...
  const fullPath = path.join(
    attackTrailTextureDirectory(projectStore, project, payload.profileId, payload.animationId),
    `${hash}.png`,
  );
...
function validateAttackTrails(data, manifest) {
  ...
  for (const profile of manifest?.profiles || []) {
    for (const animation of profile.animations || [])
      profileAnimations.add(`${profile.id}/${animation.id || animation.name}`);
  }
