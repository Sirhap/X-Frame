...
function run(argv = process.argv.slice(2)) {
  ...
  const projects = args.project
    ...
    : registry.projects;
  if (!projects.length)
    throw new Error(args.project ? `Lite project not found: ${args.project}` : "No Lite projects found.");
  const reports = [];
    ...
    for (const profile of manifest.profiles || []) {
      for (const animation of profile.animations || []) {
        ...
        for (const frame of animation.frames || []) {
          ...
          if (!full.startsWith(`${target.workspaceDir}${path.sep}`) && full !== target.workspaceDir)
            warnings.push(
              `${profile.id}/${animation.id}: frame is outside the isolated Lite workspace: ${frame.path}`,
            );
          if (full && !fs.existsSync(full))
            warnings.push(`${profile.id}/${animation.id}: missing stable frame: ${frame.path}`);
        }
      }
    }
    reports.push({
