(function frameTunerLiteUi() {
  ...
  function syncSettings() {
    ...
    state.layout =
      settings.canvas?.autoMeasured === true
        ? {
  ...
  async function calculateOptimalLayout(api, batch, config, status) {
    ...
    try {
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        if (!union) {
          if (probeWidth >= 8192 && probeHeight >= 8192)
            throw new Error("当前角色的所有动作都没有找到可见像素。");
        ...
        const touchesEdge =
          union.left <= 2 ||
          union.top <= 2 ||
          union.right >= probeWidth - 3 ||
          union.bottom >= probeHeight - 3;
        if (touchesEdge) {
          if (probeWidth >= 8192 && probeHeight >= 8192)
            throw new Error("当前角色所有动作的总可见范围超过 8192 px。");
          probeWidth = Math.min(8192, probeWidth * 2);
          probeHeight = Math.min(8192, probeHeight * 2);
          continue;
        }
        ...
        if (width > 8192 || height > 8192)
          throw new Error(`全角色最优画布 ${width}×${height} 超过 8192 px。`);
        return {
          width,
          height,
  ...
  function sheetJson(metadataFrames, sheet, phaseDurationMs, canvas, audio) {
    ...
    return {
      frames: Object.fromEntries(
        metadataFrames.map((frame, index) => [
          frame.filename,
          {
  ...
  function audioMetadata(target, packagedAudio) {
    const events = (target.audio?.events || [])
      .map((event) => {
    ...
    const filesById = new Map(
      [...packagedAudio.values()].filter((asset) => ids.has(asset.id)).map((asset) => [asset.id, asset]),
    );
    return {
      schemaVersion: 1,
  ...
  async function exportOutput(kind) {
    ...
    try {
      ...
      for (let groupIndex = 0; groupIndex < batch.targets.length; groupIndex += 1) {
        ...
        const targetDirectory = await batchDirectory.getDirectoryHandle(
          safeFolderName(selected.animationId),
          { create: true },
        );
        ...
        if (
          kind === "sheet" &&
          (sheetWidth > 16384 || sheetHeight > 16384 || sheetWidth * sheetHeight > 120000000)
        ) {
