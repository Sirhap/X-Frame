...
function frameEntries(raw) {
  if (Array.isArray(raw?.frames))
    return raw.frames.map((value, index) => [String(value.filename || value.name || index), value]);
  if (raw?.frames && typeof raw.frames === "object")
    return Object.entries(raw.frames).sort(([left], [right]) => naturalCompare(left, right));
...
function cropFromEntry(value) {
  ...
  if (![x, y, width, height].every(Number.isFinite) || width <= 0 || height <= 0)
    throw new Error(`Invalid sheet frame: ${JSON.stringify(frame)}`);
...
function importSheetAudio({
  ...
  const retained = (Array.isArray(existing) ? existing : []).filter((binding) => {
    ...
    return !(
      String(metadata.projectId || project.id) === project.id &&
  ...
  for (const event of Array.isArray(source.audio.events) ? source.audio.events : []) {
    ...
    const asset =
      files.get(`id:${String(event.assetId || "")}`) || files.get(`file:${String(event.file || "")}`);
