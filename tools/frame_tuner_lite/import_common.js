...
function animationDestination(project, profileId, animationId) {
  return path.join(
    store.paths(project).workspaceDir,
    "assets",
    slug(profileId, "sequence"),
...
function saveAnimation({ project, profileId, profileLabel, animation }) {
  ...
  const existingIndex = profile.animations.findIndex(
    (entry) => String(entry.id || entry.name) === animation.id,
  );
  ...
  if (animation.previewOwner) {
    const owner = profile.animations.find(
      (entry) => String(entry.id || entry.name) === animation.previewOwner,
    );
