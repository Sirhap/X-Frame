...
const vector = (value, fallback = { x: 0, y: 0 }) => ({
  x: Number(value?.x ?? fallback.x),
  y: Number(value?.y ?? fallback.y),
});
const scaleVector = (value, fallback = 1) => ({
...
function send(res, status, body, contentType = "application/json") {
  const data = Buffer.isBuffer(body)
    ? body
    : Buffer.from(
        /^application\/json(?:\s*;|$)/i.test(contentType) ? JSON.stringify(body, null, 2) : String(body),
      );
...
function normalizeManifest(raw) {
  return {
    ...
    profiles: (Array.isArray(raw?.profiles) ? raw.profiles : []).map((profile) => ({
      ...
      supports: Array.isArray(profile.supports)
        ? profile.supports
        : ["character_transform", "group_transform", "frame_transform", "frame_playback", "reference_frame"],
...
function frameForClient(frame, index) {
  ...
  return {
    ...
    crop:
      frame.crop && typeof frame.crop === "object"
        ? {
            ...
            sheetWidth: Number(frame.crop.sheetWidth || size.width || 0),
            sheetHeight: Number(frame.crop.sheetHeight || size.height || 0),
          }
        : null,
  ...
  for (const profile of manifest.profiles) {
    for (const animation of profile.animations) {
      ...
      groups.push({
        ...
        characterScale: `${characterKey}.visual_size`,
        characterScaleVector: `${characterKey}.visual_scale`,
        characterOffset: `${characterKey}.offset`,
        characterRotation: `${characterKey}.rotation`,
        ...
        runtimeScale: Number(profile.runtimeScale || 1),
        bodyScale: Number(profile.bodyScale || 1),
        scale: `${groupKey}.visual_size`,
        scaleVector: `${groupKey}.visual_scale`,
        offset: `${groupKey}.offset`,
        rotation: `${groupKey}.rotation`,
        defaultScale,
  ...
  const settings = {
    ...
    canvas:
      rawSettings.canvas && typeof rawSettings.canvas === "object"
        ? rawSettings.canvas
        : { ...EMPTY_SETTINGS.canvas },
    export: {
      phaseDurationMs: Math.min(
        1000,
        Math.max(1, Math.round(Number(rawSettings.export?.phaseDurationMs || 80))),
      ),
...
function validateLiteProject(project, data = projectData(project)) {
    ...
    for (const animation of profile.animations) {
      ...
      for (const [index, frame] of (animation.frames || []).entries()) {
        ...
        if (!full || !fs.existsSync(full))
          warnings.push(`${key}: 缺少第 ${index + 1} 帧：${frame.path || "(empty)"}`);
      }
    }
  }
  ...
  if (
    data.settings.canvas?.autoMeasured === true &&
    (width < 1 || height < 1 || width > 8192 || height > 8192)
  )
    warnings.push("已计算的统一透明画布尺寸必须在 1-8192 px。 ");
  return warnings;
}
...
function configResponse(projectId) {
  ...
  if (!project) {
    return {
      root: ROOT,
      workspaceRoot: path.join(ROOT, "workspace", "lite"),
      workspaceAllRoot: path.join(ROOT, "workspace", "lite"),
      projectRoot: "",
      activeProjectId: "",
      activeProject: null,
      projects,
      ...
      tuning: clone(EMPTY_TUNING),
      warnings: ["Lite 还没有素材。请让 Agent 导入 PNG 序列或 PNG+JSON sheet。"],
      references: {},
      projectKind: "frame_lite",
      liteSettings: clone(EMPTY_SETTINGS),
      groups: [],
    };
  }
  ...
  return {
    root: ROOT,
    workspaceRoot: data.target.workspaceDir,
    workspaceAllRoot: path.join(ROOT, "workspace", "lite"),
    projectRoot: "",
    activeProjectId: project.id,
    activeProject: store.projectForClient(project),
    projects,
    scenes: [],
    profiles: data.manifest.profiles.map((profile) => ({
      id: profile.id,
      label: profile.label,
      kind: profile.kind,
    ...
    frameAudioBindings: data.audio,
    frameImageAttachments: data.attachments,
    attackTrails: data.attackTrails,
    tuning: {
      ...data.tuning.values,
      scene_settings: data.tuning.scene_settings || {},
      frame_visual_overrides: data.tuning.frame_visual_overrides || {},
      frame_playback_overrides: data.tuning.frame_playback_overrides || {},
      frame_box_overrides: data.tuning.frame_box_overrides || {},
    },
    tuningDefaults: {},
    bossTuning: {},
    ...
    yechengPropTuning: {},
    yechengPropDefaults: {},
    warnings: validateLiteProject(project, data),
    references: {},
    projectKind: "frame_lite",
    liteSettings: data.settings,
...
function saveAttachmentImage(project, payload) {
  ...
  return {
    path: reslash(path.relative(ROOT, full)),
    assetHash: hash,
    name: String(payload.name || path.basename(full)),
    type: match[1].toLowerCase(),
    width: Number(payload.width || size.width || 0),
    height: Number(payload.height || size.height || 0),
  };
}
...
function replaceFrame(project, payload) {
  ...
  if (!full || !isInside(full, workspace) || path.extname(full).toLowerCase() !== ".png" || !match)
    throw new Error("Frame replacement must be a PNG inside the active Lite workspace.");
...
function savePayload(project, payload) {
  ...
  const audio = saveFrameAudioBindings(
    project,
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/config")
      return send(res, 200, configResponse(url.searchParams.get("project")));
    ...
    if (req.method === "POST" && url.pathname === "/api/replace-animation") {
      ...
      if (!frames.length || frames.length !== files.length)
        return send(res, 400, { error: "Replacement PNG count must match the animation frame count." });
      return send(res, 200, {
        ok: true,
        frames: frames.map((frame, index) =>
          replaceFrame(project, { path: frame.path, data: files[index].data }),
        ),
      });
    }
    if (req.method === "POST" && url.pathname === "/api/lite/settings") {
      ...
      const settings = {
        ...
        export: {
          phaseDurationMs: Math.min(
            1000,
            Math.max(1, Math.round(Number(payload.export?.phaseDurationMs || 80))),
          ),
