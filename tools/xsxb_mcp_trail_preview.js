...
const { createSoftwareDom, SoftwareImage } = require("./xsxb_software_canvas");
const {
...
function attachmentsForFrame(bindings, bindingKey, frameIndex) {
  ...
  const animationId = String(bindingKey || "").split("/").slice(1).join("/") || String(bindingKey || "");
  return (Array.isArray(bindings) ? bindings : []).filter((entry) => {
    ...
    const frame = Number(
      Number.isFinite(Number(parsed?.frame)) ? parsed.frame : entry.frame ?? meta.frame,
    );
