...
function profileSceneAliases(profile) {
  ...
  const add = (value) => {
    ...
    const withoutDisplaySuffix = normalized.replace(
      /_(?:idle|actions?|animations?|frames?|sprite_frames?)$/,
      "",
    );
...
function relevantSceneText(sceneText) {
  return String(sceneText || "")
    ...
    .filter((line) => {
      ...
      return (
        /type="Script"/i.test(line) && /(?:controller|actor|character|player|boss)[^\"]*\.gd"/i.test(line)
      );
    })
    .join("\n");
}
