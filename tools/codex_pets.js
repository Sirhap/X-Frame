...
const { createCodexPetLifecycle } = require("./codex_pet_lifecycle");
...
function extractBuiltinPets(asarPath, outputDir, previous = {}) {
  ...
  const entries = walkAsar(header).filter((entry) =>
    /\/webview\/assets\/[^/]+-spritesheet-v\d+-[^/]+\.webp$/i.test(entry.path),
  );
  const fd = fs.openSync(asarPath, "r");
  const pets = [];
  try {
    ...
    for (const entry of entries) {
      ...
      const current =
        ...
        fs.statSync(destination).size === Number(entry.size || 0) &&
      ...
      const metadata = BUILTIN_PETS[id] || {
...
function discoverCustomPets(petRoot, outputDir, previous = {}) {
  ...
  for (const entry of fs.readdirSync(petRoot, { withFileTypes: true })) {
    ...
    pets.push({
      ...
      hasBackup: fs.existsSync(path.join(petDir, "spritesheet.xsxb-backup.webp")),
    });
  }
...
function syncCodexPetProject(root, projectStore, project) {
  ...
  const builtins = asarPath
    ...
    : [];
  const custom = discoverCustomPets(
    project.petRoot || codexPetRoot(),
    path.join(workspaceDir, "spritesheets", "custom"),
    previousById,
  );
...
function decodeWebpDataUrl(dataUrl) {
  ...
  if (size.width !== ATLAS_WIDTH || ![ATLAS_HEIGHT, ATLAS_HEIGHT_V2].includes(size.height)) {
    throw new Error(
      `Codex pet atlas must be ${ATLAS_WIDTH}x${ATLAS_HEIGHT} (v1) or ${ATLAS_WIDTH}x${ATLAS_HEIGHT_V2} (v2); received ${size.width}x${size.height}.`,
    );
  }
  return buffer;
}
...
function importCodexPet(project, payload) {
  if (!project || project.kind !== "codex_pets")
    throw new Error("The active project is not the Codex Pets project.");
...
function exportCodexPet(projectStore, project, payload) {
  ...
  if (!pet || !pet.writable || !pet.installedSpritesheetPath)
    throw new Error(`Codex pet is read-only or missing: ${profileId}`);
  const buffer = decodeWebpDataUrl(payload.data);
  ...
  if (!relative || relative.startsWith("..") || path.isAbsolute(relative))
    throw new Error("Codex pet spritesheet path escaped its pet directory.");
...
const { removeCustomCodexPet, restoreCodexPetBackup, restoreRemovedCodexPet } = createCodexPetLifecycle({
  codexPetRoot,
  parseWebpSize,
  atlasWidth: ATLAS_WIDTH,
  atlasHeights: [ATLAS_HEIGHT, ATLAS_HEIGHT_V2],
});
