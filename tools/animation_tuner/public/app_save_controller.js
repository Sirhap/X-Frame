(function attachXsxbAppSave(root, factory) {
   ...
   *   collectReferenceFrameForSave?:()=>object|null,
  ...
  function createController(dependencies = {}) {
    const {
      ...
      collectReferenceFrameForSave = () => null,
    ...
    async function ensureCollisionBoxOverridesForSave() {
      ...
      for (const group of config.groups) {
        ...
        for (let index = 0; index < group.frames.length; index += 1) {
          ...
          const hadCollisionBox = Boolean(entry.collisionbox);
          ...
          if (!hadCollisionBox) entry.autoGenerated = true;
          store[key] = entry;
        }
      }
    }
      ...
      try {
        ...
        const res = await fetchImpl("/api/save", {
          ...
          body: JSON.stringify({
            ...
            reference_frame: collectReferenceFrameForSave(),
    ...
    /**
     * Serializes callers onto the active save request so autosave cannot mistake
     * an ignored request for a completed persistence operation.
     * @returns {Promise<void|undefined>} Active or newly started save request.
     */
    function save() {
      if (inFlightSavePromise) return inFlightSavePromise;
      if (getSaveInFlight()) return Promise.reject(new Error("A save request is already running."));
      inFlightSavePromise = saveOnce().finally(() => {
