(function attachXsxbAppFrameEditState(root, factory) {
  ...
  function createController(dependencies = {}) {
    ...
    /** Returns whether a box payload already has a usable x/y vector. */
    function hasBoxVector(value) {
      return Boolean(value && typeof value === "object" && (value.x != null || value.y != null));
    }
    ...
    function setBoxOverride(boxName, box, index = getSelectedFrame(), group = getCurrentGroup()) {
      ...
      if (boxName === "collisionbox") delete entry.autoGenerated;
      const incoming = box || {};
      const needsCurrent = !hasBoxVector(incoming.offset) || !hasBoxVector(incoming.size);
      const current = needsCurrent ? frameBox(boxName, index, group, getImages()) : incoming;
      const merged = {
        offset: hasBoxVector(incoming.offset) ? incoming.offset : current.offset,
        size: hasBoxVector(incoming.size) ? incoming.size : current.size,
