(function attackTrailModule() {
  ...
  /** Opacity used for key-stick poses authored on frames other than the selected frame. */
  const REFERENCE_STICK_OPACITY = 0.32;
  ...
  const normalizeColor = (value, fallback = "#d9364a") =>
  ...
  const colorChannels = (value) =>
    [1, 3, 5].map((offset) => parseInt(normalizeColor(value).slice(offset, offset + 2), 16));
  const channelsToColor = (channels) =>
    `#${channels
      .map((channel) =>
        Math.round(clamp(channel, 0, 255, 0))
          .toString(16)
          .padStart(2, "0"),
      )
      .join("")}`;
  ...
  class AttackTrailEditor {
    constructor(hooks) {
      ...
      this.els = Object.fromEntries(
        [
          ...
          "attackTrailNew",
          "attackTrailDelete",
          "attackTrailDeleteHint",
          "attackTrailLayerToggle",
          "attackTrailTextureFile",
          "attackTrailTextureBrowse",
          "attackTrailTextureReset",
          "attackTrailTextureName",
          "attackTrailTexturePreview",
          "attackTrailColorMode",
          "attackTrailColor",
          "attackTrailPickColor",
          "attackTrailGradientEditor",
          "attackTrailGradientBar",
          "attackTrailGradientColor",
          "attackTrailGradientDelete",
          "attackTrailGradientPosition",
          "attackTrailBeforeTimeMs",
          "attackTrailAfterTimeMs",
          "attackTrailTailSamples",
          "attackTrailPathColumns",
          "attackTrailTailFadeStart",
          "attackTrailHeadCurvature",
          "attackTrailHeadCurvatureValue",
          "attackTrailAddStick",
          "attackTrailDeleteStick",
          "attackTrailReverse",
          "attackTrailTimingSummary",
          "attackTrailGuideToggle",
          "attackTrailPresetDialog",
          "attackTrailPresetForm",
          "attackTrailPresetName",
          "attackTrailPresetCancel",
    ...
    isContinuous() {
      return (
        this.enabled &&
        this._segments().some(
          (segment) => segment.enabled !== false && segment.generated !== false && segment.sticks.length >= 2,
        )
      );
    }
    ...
    async prepareExport() {
      await Promise.all(
        this._segments()
          .filter(
            (segment) => segment.enabled !== false && segment.generated !== false && segment.texture?.path,
          )
          .map(async (segment) => {
    ...
    render() {
      ...
      for (const element of [
        e.attackTrailDelete,
        e.attackTrailLayerToggle,
        e.attackTrailTextureBrowse,
        e.attackTrailTextureReset,
        e.attackTrailColorMode,
        e.attackTrailColor,
        e.attackTrailPickColor,
        e.attackTrailGradientColor,
        e.attackTrailBeforeTimeMs,
        e.attackTrailAfterTimeMs,
        e.attackTrailTailSamples,
        e.attackTrailPathColumns,
        e.attackTrailTailFadeStart,
        e.attackTrailHeadCurvature,
        e.attackTrailAddStick,
        e.attackTrailDeleteStick,
        e.attackTrailReverse,
      ]) {
    ...
    drawGuides() {
      ...
      const selectedFrame = this.hooks.selectedFrame();
      ...
      for (const stick of segment.sticks) {
        ...
        const currentFrame = stick.frame === selectedFrame;
        const selected = currentFrame && stick.id === this.stickId;
        const layerColor = stick.layer === "front" ? "#ff982e" : "#39baff";
        ctx.save();
        ctx.globalAlpha = currentFrame ? (selected ? 1 : 0.82) : REFERENCE_STICK_OPACITY;
        ...
        for (const handle of [top, bottom]) {
          ctx.beginPath();
          ctx.arc(handle.x, handle.y, HANDLE_RADIUS * dpr, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.save();
        ctx.shadowBlur = 0;
        ...
        ctx.lineWidth = 3 * dpr;
        ctx.beginPath();
        ctx.arc(center.x, center.y, CENTER_HANDLE_RADIUS * dpr, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = layerColor;
        ctx.beginPath();
        ctx.arc(center.x, center.y, 2.5 * dpr, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
        ctx.strokeStyle = layerColor;
        ctx.fillStyle = layerColor;
        ...
        if (selected) {
          ...
          ctx.lineWidth = 3 * dpr;
          ctx.beginPath();
          ctx.arc(arrow.x, arrow.y, 7 * dpr, 0, Math.PI * 2);
          ctx.fill();
          ctx.stroke();
          ctx.restore();
        }
    ...
    _drawSelectedPathGuide(segment) {
      ...
      const nextPose =
        selectedIndex + 1 < segment.sticks.length ? this._stickPose(segment.sticks[selectedIndex + 1]) : null;
      const distances = [previousPose, nextPose]
        .filter(Boolean)
        .map((entry) => Math.hypot(entry.center.x - pose.center.x, entry.center.y - pose.center.y));
      ...
      const outgoing = this.hooks.localToScreen({
        x: pose.center.x + direction.x * handleDistance,
        y: pose.center.y + direction.y * handleDistance,
      });
      const incoming = this.hooks.localToScreen({
        x: pose.center.x - direction.x * handleDistance,
        y: pose.center.y - direction.y * handleDistance,
      });
      ctx.setLineDash([]);
      ...
      ctx.beginPath();
      ctx.moveTo(incoming.x, incoming.y);
      ctx.lineTo(outgoing.x, outgoing.y);
      ctx.stroke();
      for (const handle of [incoming, outgoing]) {
        ctx.beginPath();
        ctx.arc(handle.x, handle.y, 4 * dpr, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.beginPath();
    ...
    pointerDown(event) {
      ...
      for (const stick of [...this._frameSticks(segment)].reverse()) {
        ...
        if (
          ...
          CENTER_HANDLE_RADIUS * this.hooks.dpr() * 1.8
        ) {
    ...
    pointerMove(event) {
      ...
      if (this.drag.mode === "direction") {
        ...
        if (desiredLength > 0.001) {
          ...
          const degrees =
            (Math.atan2(
              base.x * normalized.y - base.y * normalized.x,
              base.x * normalized.x + base.y * normalized.y,
            ) *
              180) /
            Math.PI;
        ...
        stick.top = {
          x: Math.round((this.drag.startTop.x + dx) * 10) / 10,
          y: Math.round((this.drag.startTop.y + dy) * 10) / 10,
        };
        stick.bottom = {
          x: Math.round((this.drag.startBottom.x + dx) * 10) / 10,
          y: Math.round((this.drag.startBottom.y + dy) * 10) / 10,
        };
      } else {
    ...
    _bind() {
      ...
      e.attackTrailTextureReset.addEventListener("click", () => this._resetTexture());
      e.attackTrailTextureFile.addEventListener("change", () =>
        this._uploadTexture(e.attackTrailTextureFile.files?.[0]),
      );
      e.attackTrailColorMode.addEventListener("change", () =>
        this._setColorMode(e.attackTrailColorMode.value),
      );
      e.attackTrailColor.addEventListener("input", () =>
        this._editSegment("color", e.attackTrailColor.value),
      );
      e.attackTrailGradientColor.addEventListener("pointerdown", () => {
        if (this._segment()?.colorMode === "gradient" && this._gradientStop())
          this.hooks.pushUndo("change attack trail gradient color");
      });
      e.attackTrailGradientColor.addEventListener("input", () =>
        this._setGradientStopColor(e.attackTrailGradientColor.value),
      );
      e.attackTrailGradientDelete.addEventListener("click", () => this._deleteGradientStop());
      ...
      e.attackTrailPickColor.addEventListener("click", () => {
      ...
      for (const [id, key, min, max] of [
        ["attackTrailTailSamples", "tailSamples", 4, 8],
        ["attackTrailPathColumns", "pathColumns", 8, 96],
      ])
        e[id].addEventListener("input", () => this._editSegment(key, clamp(e[id].value, min, max, min)));
      e.attackTrailBeforeTimeMs.addEventListener("input", () =>
        this._editChaseTime("before", e.attackTrailBeforeTimeMs.value),
      );
      e.attackTrailAfterTimeMs.addEventListener("input", () =>
        this._editChaseTime("after", e.attackTrailAfterTimeMs.value),
      );
      e.attackTrailTailFadeStart.addEventListener("input", () =>
        this._editSegment("tailFadeStart", clamp(e.attackTrailTailFadeStart.value, 0, 95, 60) / 100),
      );
      e.attackTrailHeadCurvature.addEventListener("input", () =>
        this._editSegment("headCurvature", clamp(e.attackTrailHeadCurvature.value, -100, 100, 0) / 100),
      );
    ...
    _bindingKey() {
      ...
      if (!group) return "";
      const canonicalKey = group.runtimeAnimation || `${group.profileId}/${group.animationId || group.name}`;
      if (this.data.bindings[canonicalKey]) return canonicalKey;
      const legacyKey = `${group.profileId}/${group.name}`;
      const legacyKeyConflicts = Array.from(this.hooks.groups?.() || []).some(
        (candidate) => candidate !== group && candidate.runtimeAnimation === legacyKey,
      );
      return !legacyKeyConflicts && this.data.bindings[legacyKey] ? legacyKey : canonicalKey;
    }
    ...
    /** Returns the project-level default texture used by reset actions. */
    _defaultTexture() {
      return clone(this.data.presetTexture || DEFAULT_PRESET_TEXTURE);
    }

    /** Reports whether a segment already uses the project-level default texture. */
    _usesDefaultTexture(segment = this._segment()) {
      if (!segment?.texture?.path) return false;
      const fallback = this.data.presetTexture || DEFAULT_PRESET_TEXTURE;
      if (segment.texture.assetHash && fallback.assetHash) {
        return segment.texture.assetHash === fallback.assetHash;
      }
      return segment.texture.path === fallback.path;
    }
    ...
    _beginPresetEdit(segment) {
      ...
      if (
        ...
        this.presetEditOriginal?.segmentId === segment.id
      )
        return;
      this._discardPresetEdit();
    ...
    _savePreset(name) {
      ...
      const presetName =
        String(name || "")
          .trim()
          .slice(0, 60) || this._defaultPresetName();
      const segment = this._normalizeSegment(
        {
          ...style,
          id: randomId("trail"),
    ...
    _gradientColorAt(stops, position) {
      ...
      for (let index = 1; index < normalized.length; index += 1) {
        ...
        const phase = clamp(
          (sample - left.position) / Math.max(0.000001, right.position - left.position),
          0,
          1,
          0,
        );
    ...
    _renderGradientEditor(segment) {
      ...
      e.attackTrailGradientBar.innerHTML = segment.gradientStops
        .map(
          (stop) =>
      ...
      e.attackTrailGradientDelete.disabled = !selected || segment.gradientStops.length <= 2;
      e.attackTrailGradientDelete.title =
        segment.gradientStops.length <= 2 ? "渐变至少保留两个颜色节点" : "移除当前选中的颜色节点";
      if (selected) {
    ...
    _gradientPointerDown(event) {
      ...
      if (stopButton) {
        ...
        const stop = {
          id: randomId("gradient"),
          position,
          color: channelsToColor(this._gradientColorAt(segment.gradientStops, position)),
        };
    ...
    _gradientPointerMove(event) {
      ...
      const segment = this._segment(),
        stop = this._gradientStop(segment);
    ...
    _gradientPointerUp(event) {
      if (!this.gradientDrag || this.gradientDrag.pointerId !== event.pointerId) return;
      this.gradientDrag = null;
      if (this.els.attackTrailGradientBar.hasPointerCapture(event.pointerId))
        this.els.attackTrailGradientBar.releasePointerCapture(event.pointerId);
    }
    ...
    _setGradientStopColor(value) {
      ...
      this.processed.clear();
      this.hooks.markDirty();
      this._renderGradientEditor(this._segment());
      this.hooks.draw();
    }
    ...
    _editChaseTime(phase, rawValue) {
      ...
      if (phase === "before") {
        ...
        this._editSegment(
          "beforeStopChaseMultiplier",
          clamp(1 - beforeMs / times.durationMs, 0, 1, DEFAULT_BEFORE_CHASE_MULTIPLIER),
        );
        return;
      }
      ...
      this._editSegment(
        "afterStopChaseMultiplier",
        clamp(times.beforeMs / afterMs, 0.1, 20, DEFAULT_AFTER_CHASE_MULTIPLIER),
      );
    }
    ...
    _addStick() {
      ...
      if (!segment) return;
      if (segment.presetOnly && !this._isPresetSegment(selected))
        segment = this._trailFromSavedPreset(segment);
      ...
      if (previous && next && previous.frame < frame && next.frame > frame) {
        ...
        top = {
          x: previous.top.x + (next.top.x - previous.top.x) * phase,
          y: previous.top.y + (next.top.y - previous.top.y) * phase,
        };
        bottom = {
          x: previous.bottom.x + (next.bottom.x - previous.bottom.x) * phase,
          y: previous.bottom.y + (next.bottom.y - previous.bottom.y) * phase,
        };
      } else if (previous) {
    ...
    _renumberAndAutoPhase(segment) {
      ...
      for (const sticks of frames.values()) {
        ...
        while (index < sticks.length) {
          if (sticks[index].phaseMode === "manual") {
            index += 1;
            continue;
          }
          const start = index;
        ...
        sticks.forEach((stick, stickIndex) => {
          ...
          stick.framePhase = clamp(
            stick.framePhase,
            previous + 0.0001,
            maximum,
            (stickIndex + 1) / (sticks.length + 1),
          );
      ...
      try {
        ...
        const response = await fetch("/api/attack-trail-texture", {
          ...
          body: JSON.stringify({
            ...
            animationId: this.hooks.group().animationId || this.hooks.group().name,
            name: file.name,
            data,
          }),
        });
    ...
    /** Restores the selected segment to the built-in project texture and keeps the action undoable. */
    _resetTexture() {
      const current = this._segment();
      if (!current || this._usesDefaultTexture(current)) return;
      this.hooks.pushUndo("restore default attack trail texture");
      const segment = this._materializePreset();
      if (!segment) return;
      this._beginPresetEdit(segment);
      const previousPath = segment.texture?.path;
      segment.texture = this._defaultTexture();
      this._updateGenerated(segment);
      if (previousPath) this.images.delete(previousPath);
      this.processed.clear();
      this.pathCache.clear();
      this.hooks.markDirty();
      this.render();
    ...
    _pickColor(event) {
      ...
      if (x < 0 || y < 0 || x >= image.width || y >= image.height)
        return this.hooks.status("请在角色 Sprite 的有效像素上取色。", true);
    ...
    _timing(segment) {
      ...
      for (let index = 1; index < absolute.length; index += 1)
        absolute[index] = Math.max(absolute[index], absolute[index - 1] + 0.0001);
    ...
    _pathState(segment) {
      ...
      for (let index = 0; index < segment.tailSamples; index += 1)
        speeds.push(SPEED_PROFILE[index] + (random() * 2 - 1) * variation);
      const mean = speeds.reduce((sum, value) => sum + value, 0) / speeds.length;
      const state = {
        signature,
        timing,
        samples,
        total: Math.max(0.001, distance),
        speeds: speeds.map((value) => value / mean),
      };
    ...
    _pose(sticks, times, time) {
      ...
      const a = sticks[index],
        b = sticks[index + 1],
        pa = this._stickPose(a),
        pb = this._stickPose(b);
      const t = clamp((time - times[index]) / Math.max(0.0001, times[index + 1] - times[index]), 0, 1, 0);
      const da = this._direction(a),
        db = this._direction(b);
    ...
    _directionAtTime(sticks, times, time) {
      ...
      const a = this._direction(sticks[index]),
        b = this._direction(sticks[index + 1]);
    ...
    _directionHandleScreen(stick) {
      ...
      const center = this.hooks.localToScreen(pose.center);
      const direction = this._direction(stick);
      const sample = this.hooks.localToScreen({
        x: pose.center.x + direction.x,
        y: pose.center.y + direction.y,
      });
      ...
      return {
        ...
        arrow: {
          x: center.x + screenDirection.x * arrowLength,
          y: center.y + screenDirection.y * arrowLength,
        },
      };
    }
    ...
    _distanceAtTime(state, time) {
      return this._interpolateSamples(state.samples, "time", "distance", time);
    }
    _timeAtDistance(state, distance) {
      return this._interpolateSamples(state.samples, "distance", "time", distance);
    }
    _interpolateSamples(samples, key, valueKey, target) {
      ...
      while (low + 1 < high) {
        ...
        if (samples[middle][key] <= target) low = middle;
        else high = middle;
      }
      const span = samples[high][key] - samples[low][key];
      const f = span <= 1e-8 ? 0 : (target - samples[low][key]) / span;
    ...
    _drawSegment(segment, alpha, layer) {
      ...
      if (!image) {
        this.hooks
          .loadTexture(segment.texture)
          .then((loaded) => {
            this.images.set(segment.texture.path, loaded);
            this.hooks.draw();
          })
          .catch(() => {});
        return;
      ...
      const animationTiming = this.hooks.animationTiming?.() || {
        duration: state.timing.absolute.at(-1),
        lastPlayableFrameStart: state.timing.absolute.at(-1),
      };
      const lastArrival = state.timing.absolute.at(-1);
      const finishAt =
        animationTiming.lastPlayableFrameStart > lastArrival + 0.0001
      ...
      const tails = this._tailDistances(
        state,
        segment,
        currentDistance,
        catchElapsed,
      ...
      const capCatchDuration =
        (duration * Math.max(0, 1 - segment.beforeStopChaseMultiplier)) /
      ...
      for (let row = 0; row < rows - 1; row += 1)
        for (let column = 0; column < columns - 1; column += 1) {
          ...
          const v0 = (row / (rows - 1)) * texture.height,
            v1 = ((row + 1) / (rows - 1)) * texture.height;
          const triangles = [
            {
              source: [
                { x: u0, y: v0 },
              ...
              target: [grid[row][column], grid[row][column + 1], grid[row + 1][column + 1]],
              times: [gridTimes[row][column], gridTimes[row][column + 1], gridTimes[row + 1][column + 1]],
            },
            {
              source: [
                { x: u0, y: v0 },
              ...
              target: [grid[row][column], grid[row + 1][column + 1], grid[row + 1][column]],
              times: [gridTimes[row][column], gridTimes[row + 1][column + 1], gridTimes[row + 1][column]],
            },
          ];
          let cellVisible = false;
          for (const triangle of triangles) {
            const triangleTime =
              triangle.times.reduce((sum, value) => sum + value, 0) / triangle.times.length;
            if (this._layerAtTime(segment.sticks, state.timing.times, triangleTime, segment.layer) !== layer)
              continue;
            cellVisible = true;
    ...
    _drawGpuMesh(textureSource, grid, gridTimes, segment, state, layer, width, height) {
      ...
      const appendTriangle = (a, b, c, timeA, timeB, timeC) => {
        ...
        if (this._layerAtTime(segment.sticks, state.timing.times, sampleTime, segment.layer) !== layer)
          return;
      ...
      for (let row = 0; row < rows - 1; row += 1) {
        for (let column = 0; column < columns - 1; column += 1) {
          ...
          appendTriangle(
            ...
            topRight,
            bottomRight,
            gridTimes[row][column],
            gridTimes[row][column + 1],
            gridTimes[row + 1][column + 1],
          );
          appendTriangle(
            topLeft,
            bottomRight,
        ...
        if (segment.colorMode !== "original") {
          ...
          const tint =
            segment.colorMode === "gradient"
              ? this._gradientColorAt(segment.gradientStops, gradientPosition)
              : solidTint;
          const luma =
          ...
          pixels.data[index] = tint[0] * detail;
          pixels.data[index + 1] = tint[1] * detail;
          pixels.data[index + 2] = tint[2] * detail;
          pixels.data[index + 3] = pixels.data[index + 3] * luma;
        }
        const x = (index / 4) % canvas.width;
        const u = canvas.width <= 1 ? 0 : x / (canvas.width - 1);
        const fadeU = clamp(
          (u - segment.tailFadeStart) / Math.max(0.001, 1 - segment.tailFadeStart),
          0,
          1,
          0,
        );
    ...
    _normalizeData(raw) {
      ...
      for (const [key, values] of Object.entries(raw?.bindings || {}))
        if (Array.isArray(values) && key.includes("/"))
          bindings[key] = values.map((value, index) =>
            this._normalizeSegment(value, index, key, sourceSchema),
          );
    ...
    _normalizeSegment(value = {}, index, key, sourceSchema = ATTACK_TRAIL_SCHEMA_VERSION) {
      ...
      const segment = {
        id: String(value.id || `trail_${index + 1}`),
        name: String(value.name || `Trail ${index + 1}`),
        profileId: String(value.profileId || profileId),
        animationId: String(value.animationId || animationId),
        enabled: true,
        generated: value.generated !== false,
        presetOnly: value.presetOnly === true && (!Array.isArray(value.sticks) || value.sticks.length === 0),
        coordinateSpace: "group",
        layer: segmentLayer,
        texture: {
          path: String(value.texture?.path || ""),
          assetHash: String(value.texture?.assetHash || ""),
          name: String(value.texture?.name || ""),
          type: String(value.texture?.type || "image/png"),
          width: Number(value.texture?.width || 0),
          height: Number(value.texture?.height || 0),
          hasEffectiveAlpha: value.texture?.hasEffectiveAlpha === true,
        },
        ...
        speedVariation: clamp(value.speedVariation, 0, 0.25, 0.008),
        stableSeed: Math.round(clamp(value.stableSeed, 0, 2147483647, 73129)),
        pathColumns: Math.round(clamp(value.pathColumns, 8, 96, DEFAULT_PATH_COLUMNS)),
        pathCacheSamples: Math.round(clamp(value.pathCacheSamples, 32, 512, 192)),
        collapsedWidth: clamp(value.collapsedWidth, 0.25, 32, 2),
        sticks: (Array.isArray(value.sticks) ? value.sticks : []).map((stick, stickIndex) =>
    ...
    _chaseMultiplier(value, phase, min, max, sourceSchema = 6) {
      ...
      if (direct !== undefined && direct !== null && direct !== "") {
        ...
        if (before && sourceSchema < 6 && Math.abs(normalized - 0.7) < 0.000001)
          return DEFAULT_BEFORE_CHASE_MULTIPLIER;
        return normalized;
      }
      const legacy = Number(
        before
    ...
    _normalizeStick(value = {}, index, defaultLayer = "behind") {
      return {
        id: String(value.id || `stick_${index + 1}`),
        order: index,
        frame: Math.max(0, Math.round(Number(value.frame || 0))),
        framePhase: clamp(value.framePhase, 0, 1, 0.5),
        phaseMode: value.phaseMode === "manual" ? "manual" : "auto",
        top: point(value.top, { x: -60, y: -120 }),
        bottom: point(value.bottom, { x: 60, y: 120 }),
        reverseDirection: value.reverseDirection === true,
        directionOffset: clamp(value.directionOffset, -180, 180, 0),
        tangentStrength: clamp(value.tangentStrength, 0, 4, 0.8),
        layer: String(value.layer || defaultLayer) === "front" ? "front" : "behind",
      };
    }
    _renderTexturePreview(segment) {
      ...
      const previewPoint = (u, v) => ({
        x:
          baseX +
          (rightX - baseX) * u +
          segment.headCurvature * curvePad * this._headCurveProfile(v) * this._headCurveBlend(u),
        y: topY + drawHeight * v,
      });
      for (let row = 0; row < rows; row += 1) {
        const v0 = row / rows,
        ...
        for (let column = 0; column < columns; column += 1) {
          const u0 = column / columns,
            u1 = (column + 1) / columns;
          const source00 = sourcePoint(u0, v0),
            source10 = sourcePoint(u1, v0),
            source01 = sourcePoint(u0, v1),
            source11 = sourcePoint(u1, v1);
          const target00 = previewPoint(u0, v0),
            target10 = previewPoint(u1, v0),
            target01 = previewPoint(u0, v1),
            target11 = previewPoint(u1, v1);
      ...
      context.lineWidth = 2;
      context.beginPath();
      context.moveTo(baseX, topY);
      context.lineTo(baseX, topY + drawHeight);
      context.stroke();
      context.setLineDash([]);
